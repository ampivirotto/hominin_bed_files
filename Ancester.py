        ...
        for line in vcf:
            ...
            if line[0] != '#':
                ...
                if ancest_missing_freq >= missing_stat or (maxIndex == 4):
                    ancest_state = np.nan #when the ancestor nucleotide will be missing
                elif maxIndex == 0:
            ...
            output_df_dic[len(output_df_dic)] = add
            state_df_dic[len(state_df_dic)] = state_add #add list to dataframe for ancestor state
    pd.DataFrame.from_dict(output_df_dic, orient='index', columns=cols).to_csv('/Frequencies/Frequencies.csv',index=False) 
    pd.DataFrame.from_dict(state_df_dic,orient='index',columns=['Chromosome','Position','Nucleotide']).to_csv('/State/State.csv',index=False)
